...
const Podcast = ({ podcast, userId, isFavorite, mutex, setMutex }) => {
  ...
  const handleClick = () => {
    // Animate the component to full screen
    if(!mutex){
      setMutex(!mutex)
      setActive(!Active)
    }
  ...
  const toggleFavorite = async () => {
    try {
      if (isFavorite) {
        await axios.put(`/api/users/${userId}/favorites/remove`, { podcastId: podcast._id });
      } else {
        await axios.put(`/api/users/${userId}/favorites/add`, { podcastId: podcast._id });
      }
      onFavoriteChange();
    } catch (error) {
  ...
  return (
    ...
    className=" cursor-pointer podcast mt-24 bg-black mr-10 w-fit h-fit px-10 py-5 rounded-lg card flex flex-col gap-5">
      ...
      {podcast.type === "Audio" ? (
        <audio
          ref={mediaRef}
          src={`/api/podcasts/${podcast._id}/file`}
          controls
          onTimeUpdate={handleTimeUpdate}
        />
      ) : (
        <motion.video
        animate={{ scale: Active ? 1.5: 1 }} // scale up when active
        onClick={handleClick}
      ...
      <button onClick={toggleFavorite} className="poppins text-[#E7BF81]"><img src={Favourite} alt="" className="h-6 w-6"/></button>
    </motion.div>
  );
};
